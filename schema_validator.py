...
from app.queue.event_consumer import EventConsumer
...
from lib.schema_validate import validate
...
def _excepthook(logger, type, value, traceback):
    logger.exception("Schema validation failed", exc_info=value)


@synchronize_fail_count.count_exceptions()
def run(config, logger, session, event_consumer, shutdown_handler):

    # for now getting host from to build payload for testing.
    # TODO: 
    # 1. Build payload by reading kafka and remove this host.
    # 2. How to read events for the last hour
...
def main(logger):
    ...
    event_consumer = EventConsumer(config)
    register_shutdown(event_consumer.close, "Closing consumer")
    ...
    with session_guard(session):
        run(config, logger, session, event_consumer, shutdown_handler)
