...
from bonfire.openshift import wait_for_ready
...
def _get_base_pod_cfg():
    return {
        ...
        "spec": {
            ...
            "restartPolicy": "Never",
            "volumes": [{"name": "iqe-settings-volume", "secret": {"defaultMode": 420, "secretName": SECRET_NAME}}],
        },
    }
...
def _build_test_conf(env_parser):
    ...
    if env_parser.app_present("advisor"):
        env_conf["ADVISOR"] = {
            ...
            "engine_results_topic": env_parser.get_kafka_topic("advisor", "platform.engine.results"),
            "inventory_events_topic": env_parser.get_kafka_topic("advisor", "platform.legacy-bridge.events"),
            "payload_tracker_topic": env_parser.get_kafka_topic("advisor", "platform.payload-status"),
        ...
        if env_parser.app_present("rbac"):
            ...
            env_conf["ADVISOR"]["rbac_url"] = f"http://{hostname}:{port}/api/rbac/v1/access/?application=advisor"
...
@click.option("--env", "-e", type=str, multiple=True, help="Env var to set on container using format KEY=VAL")
